...
export const session = pgTable('session', {
  id: uuid('id').defaultRandom().primaryKey(),
  userId: uuid('user_id').notNull().references(() => user.id, { onDelete: 'cascade', onUpdate: 'cascade' }).notNull(),
...
export const productCategoryAttribute = pgTable('product_category_attribute', {
  id: uuid('id').defaultRandom().primaryKey(),
  productCategoryId: uuid('product_category_id').references(() => productCategory.id, { onUpdate: 'cascade', onDelete: 'cascade' }).notNull(),
...
export const product = pgTable('product', {
  ...
  categoryId: uuid('category_id').references(() => productCategory.id, { onUpdate: 'cascade', onDelete: 'cascade' }).notNull(),
  createdAt: timestamp('created_at', { withTimezone: true }).defaultNow().notNull(),
  updatedAt: timestamp('updated_at', { withTimezone: true }).defaultNow().notNull(),
});
...
export const productAttributeValue = pgTable('product_attribute_value', {
  id: uuid('id').defaultRandom().primaryKey(),
  productId: uuid('product_id').references(() => product.id, { onDelete: 'cascade', onUpdate: 'cascade' }).notNull(),
  attributeId: uuid('attribute_id').references(() => productCategoryAttribute.id, { onDelete: 'cascade', onUpdate: 'cascade' }).notNull(),
...
export const discount = pgTable('discount', {
  ...
  productId: uuid('product_id').references(() => product.id, { onDelete: 'cascade', onUpdate: 'cascade' }),
...
export const payment = pgTable('payment', {
  ...
  userId: uuid('user_id').references(() => user.id).notNull(),
  orderId: uuid('order_id').references(() => order.id).notNull(),
  createdAt: timestamp('created_at', { withTimezone: true }).defaultNow().notNull(),
  updatedAt: timestamp('updated_at', { withTimezone: true }).defaultNow().notNull(),
});
